...
// comment.xml과 연결되어 있는 클래스
public class SimpleComment extends AppCompatActivity {  //한줄평 클래스
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String url = "http://ipAddress/Getcomment.php?userID="+Login.userIdKey+"&order="+order;
        Log.d("url",url);
        ...
        sp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                switch (i){
                    case 1:
                        Intent intent1 = new Intent(getApplicationContext(), SimpleComment.class);
                        intent1.putExtra("order","star");
                        startActivity(intent1);
                        finish();
                        break;
                    case 2:
                        Intent intent2 = new Intent(getApplicationContext(), SimpleComment.class);
    ...
    protected void showList(){
        ...
        ArrayList<SimpleCommentInfo> simple_commentinfos = new ArrayList<>(); //관심도서 Individual 생성
        try{
            ...
            for(int i =0;i<books.length();i++){
                ...
                try {
                    mThread.join();
                    simple_commentinfos.add(new SimpleCommentInfo(bm,title,comment,star));
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
            SimpleComment_Adapter myAdapter = new SimpleComment_Adapter(simple_commentinfos,SimpleComment.this);
